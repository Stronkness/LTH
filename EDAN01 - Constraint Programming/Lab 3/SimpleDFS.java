...
public class SimpleDFS  {
	...
	int selectValue(IntVar v) {
		//Fall 1. Avrundas nedåd automatiskt
	    return (v.min() + v.max())/2;

		//Fall 2. Avrundas uppåt om inte heltal
		/**if((v.min()+v.max()/2)%2 == 0) {
			return (v.min() + v.max())/2;
		}
		else {
			return (v.min() + v.max())/2 +0.5;
		}**/
	}
	...
	public PrimitiveConstraint getConstraint() {
		//Fall 1 (mindre än eller lika med)
	    return new XlteqC(var, value);

		//Fall 2 (större än eller lika med)
