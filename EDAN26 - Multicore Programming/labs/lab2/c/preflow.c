...
void dispatch(graph_t *g, node_t *u)
{
	node_t *v;
	edge_t *e;
	list_t *p;
	...
	{
		push(g, u, v, e);
		// Why is this valid when unlocking in push is not?
		pthread_mutex_unlock(&u->mutex);
		pthread_mutex_unlock(&v->mutex);
	}
	else
...
void *push_thread(void *arg)
{
	...
	while (1)
	{
		...
		node_t *u = leave_excess(g);
		while (u == NULL)
		{
			// We could not retrieve a node with excess preflow and since
			// no other node is working there can never be more added
			if (g->working_threads == 0)
			{
				pthread_mutex_unlock(&g->mutex);
				return (void *)NULL;
			}
			pthread_cond_wait(&g->cond, &g->mutex); // Will re-acquire mutex upon waking up
			u = leave_excess(g);
		}
		g->working_threads += 1; // Make sure to notify other nodes that we are doing a dispatch
...
int preflow(graph_t *g)
{
	...
	for (i = 0; i < 7; i++)
	{
		pthread_create(&threads[i], NULL, push_thread, g);
	}
