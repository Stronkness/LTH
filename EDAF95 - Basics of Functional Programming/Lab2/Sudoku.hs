...
parseBoard result = zip squares9x9 (map digitToInt(replacePointsWithZero result))

-- Prints out all possible units. First the rows, then the columns and at last the squares (note: this solution is for a 9x9 Sudoku)
unitList :: [[String]]
unitList = [cross [r] cols9x9 | r<-rows9x9] ++ [cross rows9x9 [c] | c<-cols9x9] ++ [cross boxR boxC | boxR <- ["ABC","DEF","GHI"], boxC <- ["123","456","789"]]
...
units :: [(String, [[String]])]
units = [(x, filterUnitList x) | x<-squares9x9]
...
peers :: [(String, [String])]
peers = [(x, delete x (removeDuplicates(foldList(filterUnitList x)))) | x<-squares9x9]
...
-- Takes a input string of Sudoku numbers and checks if this board is valid and
-- if every number is also valid
...
reduceList x y = [s | s <- x, notElem s y]
...
validUnit x y = and [elem z (concat(lookups x y)) | z <- map digitToInt cols9x9]
