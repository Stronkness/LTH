...
class dispatcher(larger):
    ...
    def treatSignal(self, x, info):
        if x == ARRIVAL:
            send(ARRIVAL, simTime, self.randomDispatch(), [simTime])
...
eN = 0
for i in range(Lq):
    a = queues[i].sumMeasurements/queues[i].numberOfMeasurements
    eN += a
    print('In queueing system ',i,': ', a)
print("Mean of the total number of customers: ", eN/5)
